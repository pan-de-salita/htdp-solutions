...
(define MISSILE-VELOCITY (* TANK-VELOCITY 2))
...
;; per clock tick.
(define (si-move s)
  (si-move-proper s (random UFO-X-DELTA)))

(check-random (si-move sigs-ex0) (si-move-proper sigs-ex0 (random UFO-X-DELTA)))

;; SIGS Number -> SIGS
;; moves the space-invader objects predictably by delta.
(define (si-move-proper s delta)
  (cond [(aim? s)
         (make-aim (make-posn (+ (posn-x (aim-ufo-cor s)) delta)
                              (+ (posn-y (aim-ufo-cor s)) UFO-VELOCITY))
                   (make-tank (+ (tank-x (aim-tank-cor s)) TANK-VELOCITY)
                              TANK-Y))]
        [(fired? s)
         (make-fired (make-posn (+ (posn-x (fired-ufo-cor s)) delta)
                                (+ (posn-y (fired-ufo-cor s)) UFO-VELOCITY))
                     (make-tank (+ (tank-x (fired-tank-cor s)) TANK-VELOCITY)
                                TANK-Y)
