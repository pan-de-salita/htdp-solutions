...
;; an XCor is a Number.
;; represents the x-coordinate of an animal as it walks
;; across canvas.

;; a Joy is a Number.
;; represents the joy level of an animal.
...
;; - color
;; - joy
(define-struct VCham [x-cor color joy])

(define VCham-test-0 (make-VCham CHAM-X-START CHAM-DEFAULT-COLOR JOY-MAX))
(define VCham-test-1 (make-VCham (* 1/3 CANVAS-WIDTH) "red" (* 1/2 JOY-GAUGE-WIDTH)))
(define VCham-test-2 (make-VCham CHAM-RESET "red" JOY-MIN))
...
;; a VAnimal is one of the following:
;; - VCham
;; - VCat

;; Zoo is a structure:
;;      (make-zoo VCat VCham Focus)
;; contains both VCham and VCAt at any given time.
(define-struct zoo [VCham VCat focus])

(define zoo-test-0 (make-zoo VCham-test-0 VCat-test-0 "cham"))
(define zoo-test-1 (make-zoo VCham-test-1 VCat-test-1 "cat"))
(define zoo-test-2 (make-zoo VCham-test-2 VCat-test-2 "cat"))
...
;; - CHAM (colored)
;; - CAT (animated)
;; - JOY-GAUGE of the animal in focus
...
(define (zoo->image zoo)
  (place-images
   (list (color-VCham (zoo-VCham zoo))
         ...
         (joy-level-of zoo)
         (joy-level-of (animal-in-focus zoo)))
...
;; Zoo -> VAnimal
;; returns animal in focus.
(check-expect (animal-in-focus zoo-test-0) (zoo-VCham zoo-test-0))
(check-expect (animal-in-focus zoo-test-1) (zoo-VCat zoo-test0))

(define (animal-in-focus zoo)
