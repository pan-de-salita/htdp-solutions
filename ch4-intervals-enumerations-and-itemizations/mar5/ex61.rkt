...
(define TL-BODY (empty-scene TRAFFIC-LIGHT-BODY-WIDTH
                             TRAFFIC-LIGHT-BODY-HEIGHT
                             "black"))
...
(check-expect (traffic-light-render 0)
              (place-images (list (bulb-render-red 0)
                            ...
                            (list (make-posn TL-X-POS RED-Y-POS)
                                  (make-posn TL-X-POS GREEN-Y-POS)
                                  (make-posn TL-X-POS YELLOW-Y-POS))
                            TL-BODY))
...
(define (bulb-render-red current-state)
  (circle LIGHT-RADIUS
          (if (or (and (>= current-state RED-LIGHT) (< current-state GREEN-LIGHT))
                  (= current-state RESET))
              "solid"
              "outline")
          "red"))
...
(define (bulb-render-green current-state)
  (circle LIGHT-RADIUS
          (if (and (>= current-state GREEN-LIGHT) (< current-state YELLOW-LIGHT))
              "solid"
              "outline")
          "green"))
...
(define (bulb-render-yellow current-state)
  (circle LIGHT-RADIUS
          (if (and (>= current-state YELLOW-LIGHT) (< current-state RESET))
              "solid"
              "outline")
          "yellow"))
