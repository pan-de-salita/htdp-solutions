...
;; a BSL-da-all is a [List-of [List-of Association] [List-of BSL-fun-def]],
;; in other words, a [List-of AL BSL-fun-def*]
...
(define bda-pi
  (list (list (list 'close-to-pi 3.14)) ;; AL
        (list fd-area-of-circle fd-volume-of-10-cylinder))) ;; BSL-fun-def*
...
(define (lookup-con-def def-area con)
  (local ((define l-assoc (car def-area))
          (define lookup-con (assq con l-assoc)))
    (if (false? lookup-con)
...
(define (lookup-fun-def def-area f-name)
  (local ((define l-bsl-fun-def (cadr def-area))
          (define lookup-def
            (filter
             (lambda (f-def)
               (symbol=? (fun-def-name f-def) f-name))
             l-bsl-fun-def)))
    (if (empty? lookup-def)
