...
(define (most-frequent.v2 dict)
  (if (empty? dict)
      '()
      (largest-lc (count-by-letter.v2 (words-by-first-letter.v2 dict)))))

;; Dictionary -> List-of-Dictionaries
;; produces a List-of-Dictionaries, one per Letter.
;; CONSTRAINT: if a Dictionary within the List-of-Dictionary
;; does not contain any words for some letter, it is removed.

(check-expect (words-by-first-letter.v2 dict-test0) '())
(check-expect (words-by-first-letter.v2 dict-test1) (list dict-test1))
(check-expect (words-by-first-letter.v2 dict-test2) (list (list "c" "cat" "code") (list "a" "as")))
(check-expect
 (words-by-first-letter.v2 dict-test3)
...
(define (words-by-first-letter.v2 dict)
  (if (empty? dict)
      '()
      (remove-blanks (ldict-maker dict))))

;; Dictionary -> List-of-Dictionaries
;; produces a List-of-Dictionaries, one per Letter.

(check-expect (ldict-maker dict-test0) '())
(check-expect (ldict-maker dict-test1) (list dict-test1))
(check-expect
 (ldict-maker dict-test2)
 (list (list "c" "cat" "code")
       (list "b")
       (list "a" "as")))
(check-expect
 (ldict-maker dict-test3)
 (list (list "water" "watch" "will")
       (list "bowl" "ball")
       (list "keyboard")
       (list "pen" "pan")
...
(define (ldict-maker dict)
  (cond [(empty? dict) '()]
        [else (include-to-ldict (first dict)
                                (ldict-maker (rest dict)))]))
...
;; removes a Dictionary from a List-of-Dictionaries ldict
;; if it does not contain any words for some letter.

(check-expect (remove-blanks '()) '())
(check-expect (remove-blanks (list dict-test1)) (list dict-test1))
(check-expect
 (remove-blanks
  (list (list "c" "cat" "code")
        (list "b")
...
(define (remove-blanks ldict)
  (if (empty? ldict)
      '()
      (if (and (<= (length (first ldict)) 1) (<= (string-length (first (first ldict))) 1))
          (remove-blanks (rest ldict))
          (cons (first ldict) (remove-blanks (rest ldict))))))
...
(check-expect (count-by-letter.v2 (words-by-first-letter.v2 dict-test0)) llc-test0)
(check-expect (count-by-letter.v2 (words-by-first-letter.v2 dict-test1)) llc-test1)
(check-expect (count-by-letter.v2 (words-by-first-letter.v2 dict-test2)) (list (make-lc "c" 3) (make-lc "a" 2)))
(check-expect (count-by-letter.v2 (words-by-first-letter.v2 dict-test3)) llc-test3)
