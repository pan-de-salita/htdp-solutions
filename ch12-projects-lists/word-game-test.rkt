...
(define (insert-everywhere/in-one-word a-letter a-word initial-letter)
  (cond [(empty? a-word)
         (list (cons a-letter '()))]
        [(= (length a-word) 1)
         (list (list a-letter (first a-word)) (list (first a-word) a-letter))]
        [(string=? (first (reverse a-word)) initial-letter)
         (list (cons a-letter a-word))]
        [else (cons (cons a-letter a-word)
                    (insert-everywhere/in-one-word (first (reverse a-word))
                                                   (cons a-letter (reverse (rest (reverse a-word))))
                                                   initial-letter))]))
