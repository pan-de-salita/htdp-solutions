...
(define FOOD (circle WORM-RADIUS "solid" "white"))
...
(check-expect
 ...
 (place-image FOOD
              (posn-x (frame-food FRAME-INIT))
              (posn-y (frame-food FRAME-INIT))
              (place-image WORM-HEAD
                           (posn-x (worm-head (frame-worm FRAME-INIT)))
...
(define (game-render game-state)
  (cond [(empty? (worm-body (frame-worm game-state)))
         (place-image FOOD
                      (posn-x (frame-food game-state))
                      (posn-y (frame-food game-state))
                      (place-image WORM-HEAD
                                   (posn-x (worm-head (frame-worm game-state)))
                                   (posn-y (worm-head (frame-worm game-state)))
                                   GRID))]
        [else (place-image WORM-SEGMENT
                           (posn-x (car (worm-body (frame-worm game-state))))
                           (posn-y (car (worm-body (frame-worm game-state))))
                           (game-render (frame
                                         (worm (worm-head (frame-worm game-state))
                                               (cdr (worm-body (frame-worm game-state)))
...
(check-random (food-create (make-posn 150 100))
              (food-check-create (make-posn 150 100)
                                 (make-posn (x-coordinate-check-create (random (+ GRID-COLUMNS 1)))
...
(define (food-check-create posn-a posn-b)
  (cond [(equal? posn-a posn-b) (food-create posn-a)]
        [else posn-b]))
