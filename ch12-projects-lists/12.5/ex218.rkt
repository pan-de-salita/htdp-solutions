...
(define GAME-OVER/WALL "worm hit the wall \nyour score: ")
...
(define (worm-hurt? game-state)
  (or (worm-hit-wall? (worm-head game-state))
      (worm-hit-body? game-state)))
...
(define (worm-hit-body? game-state)
  (not (boolean? (member (head-move (worm-head game-state) (worm-direction game-state))
                         (body-move (worm-head game-state) (worm-body game-state))))))
...
(define (game-over game-state)
  (place-image (game-over-message game-state)
               (posn-x GAME-OVER-POSN)
               (posn-y GAME-OVER-POSN)
               (worm-render game-state)))

;; Posn List-of-Posns -> Image
...
(define (game-over-message game-state)
  (text (string-append
         (cond [(worm-hit-body? game-state) GAME-OVER/BODY]
               [else GAME-OVER/WALL])
         (number->string (length (worm-body game-state))))
