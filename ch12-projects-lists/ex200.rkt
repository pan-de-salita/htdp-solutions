...
;; List-of-Tracks -> Number
;; returns the total amount of play time of an List-of-Tracks

(check-expect (total-play-time/library list-of-tracks-example-0) 0)
(check-expect (total-play-time/library list-of-tracks-example-1) 162000)
(check-expect (total-play-time/library list-of-tracks-example-2) (+ 162000 428000 185000))

(define (total-play-time/library list-of-tracks)
  (cond [(empty? list-of-tracks) 0]
        [else (+ (track-time (first list-of-tracks))
                 (total-play-time/library (rest list-of-tracks)))]))

;;; application

(test)
(total-play-time/library ITUNES-TRACKS)
