...
;; does the following:
;; - if the current block has landed on GRID-BOTTOM or on
...
;; shifts the current block one column to the left or right
;; according to given Key-Event.

(check-expect
 (tetris-control tetris-0 "left")
 (tetris (block-move/left (tetris-block tetris-0))
         (tetris-landscape tetris-0)))
(check-expect
 (tetris-control tetris-0 "right")
 (tetris (block-move/right (tetris-block tetris-0))
         (tetris-landscape tetris-0)))
(check-expect
 (tetris-control
  (tetris
   block-0
   (list (block (* BLOCK-LENGTH 4) (* BLOCK-LENGTH 1))
         (block (* BLOCK-LENGTH 4) (* BLOCK-LENGTH 2))
         (block (* BLOCK-LENGTH 4) (* BLOCK-LENGTH 3))
         (block (* BLOCK-LENGTH 4) (* BLOCK-LENGTH 4))
         (block (* BLOCK-LENGTH 4) (* BLOCK-LENGTH 5))
         (block (* BLOCK-LENGTH 4) (* BLOCK-LENGTH 6))
         (block (* BLOCK-LENGTH 4) (* BLOCK-LENGTH 7))
         (block (* BLOCK-LENGTH 4) (* BLOCK-LENGTH 8))
         (block (* BLOCK-LENGTH 4) (* BLOCK-LENGTH 9))))
  "left")
 (tetris
   block-0
   (list (block (* BLOCK-LENGTH 4) (* BLOCK-LENGTH 1))
...
(check-expect
 (tetris-control
  (tetris
   block-0
   (list (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 1))
         (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 2))
         (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 3))
         (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 4))
         (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 5))
         (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 6))
         (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 7))
         (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 8))
         (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 9))))
  "right")
 (tetris
   block-0
   (list (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 1))
         ...
         (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 8))
         (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 9)))))
(check-expect (tetris-control tetris-0 "o") tetris-0)

(define (tetris-control game-state key-event)
  (tetris (cond [(and (block-space-available?/left
                       (tetris-block game-state)
                       (tetris-landscape game-state))
                      (key=? key-event "left"))
                 (block-move/left (tetris-block game-state))]
                [(and (block-space-available?/right
                       (tetris-block game-state)
                       (tetris-landscape game-state))
                      (key=? key-event "right"))
                 (block-move/right (tetris-block game-state))]
                [else (tetris-block game-state)])
          (tetris-landscape game-state)))

;; Block -> Block
;; moves the current block to the left by one column.

(check-expect (block-move/left block-0) (block (- (block-x block-0) BLOCK-LENGTH) (block-y block-0)))

(define (block-move/left current-block)
  (block (- (block-x current-block) BLOCK-LENGTH) (block-y current-block)))

;; Block -> Block
;; moves the current block to the right by one column.

(check-expect (block-move/right block-0) (block (+ (block-x block-0) BLOCK-LENGTH) (block-y block-0)))

(define (block-move/right current-block)
  (block (+ (block-x current-block) BLOCK-LENGTH) (block-y current-block)))

;; Block Landscape -> Boolean
;; checks if:
;; - there is space to the left to move the block.
;; - there is a stack of resting block to the left of
;;   the current block.

(check-expect (block-space-available?/left block-0 landscape-0) #t)
(check-expect
 (block-space-available?/left
  (block (* BLOCK-LENGTH 1) (* BLOCK-LENGTH 5))
  landscape-0)
 #f)
(check-expect
 (block-space-available?/left
...
(define (block-space-available?/left current-block current-landscape)
  (and (not (< (block-x (block-move/left current-block)) (* BLOCK-LENGTH 1)))
       (boolean? (member (block-move/left current-block)
                         current-landscape))))

;; Block Landscape -> Boolean
;; checks if:
;; - there is space to the right to move the block.
;; - there is a stack of resting block to the right of
;;   the current block.

(check-expect (block-space-available?/right block-0 landscape-0) #t)
(check-expect
 (block-space-available?/right
  (block (* BLOCK-LENGTH 9) (* BLOCK-LENGTH 5))
  landscape-0)
 #f)
(check-expect
 (block-space-available?/right
...
(define (block-space-available?/right current-block current-landscape)
  (and (not (> (block-x (block-move/right current-block)) (* BLOCK-LENGTH 9)))
       (boolean? (member (block-move/right current-block)
                         current-landscape))))

;; Game-State -> Boolean
;; checks if any of the columns are filled with blocks.

(check-expect (one-column-filled? tetris-0) #f)
(check-expect
 (one-column-filled?
  (tetris block-0
          (list (block (* BLOCK-LENGTH 6) (* BLOCK-LENGTH 1))
...
(define (one-column-filled? game-state)
  (cond [(empty? (tetris-landscape game-state)) #f]
        [else (or (= (block-y (car (tetris-landscape game-state))) GRID-HEIGHT)
                  (one-column-filled? (tetris (tetris-block game-state)
                                              (cdr (tetris-landscape game-state)))))]))
